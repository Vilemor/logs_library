...
def case_custom_message():
    return {"mensagem": "Minha mensagem customizada", "a": 2, "b": 3, "expected_msg": "Minha mensagem customizada"}


# Caso 2: Mensagem padrão
def case_default_message():
    return {"mensagem": None, "a": 5, "b": 7, "expected_msg": "Chamada da função soma"}


# Caso 3: Exceção na função
def case_exception():
    return {"mensagem": None, "a": 1, "b": 0, "raises": ZeroDivisionError}


@pytest.mark.parametrize("case_func", [case_custom_message, case_default_message])
def test_log_json_success(case_func):
    ...
    with patch("logs_library.decorator.configurar_logger") as mock_config_logger:
        ...
        mock_logger.info.assert_any_call(
            expected_msg,
            extra={"input_args": (a, b), "input_kwargs": {}}
        )
        mock_logger.info.assert_any_call(
...
def test_log_json_exception(case_func):
    ...
    with patch("logs_library.decorator.configurar_logger") as mock_config_logger:
        ...
        mock_logger.exception.assert_called_with("Exceção capturada em divide")
